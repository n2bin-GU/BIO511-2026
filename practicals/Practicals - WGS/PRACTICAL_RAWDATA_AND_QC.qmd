...
```bash
# Symlink the data directory in the n2bin-gu project dir for easy access on the HPC
# From your home directory on the HPC
ln -s  /cephyr/NOBACKUP/groups/n2bin_gu/BIO511-2026 .

# Copy the fastq files you need to your local computer as well
scp -r CID@vera2.c3se.chalmers.se:/cephyr/NOBACKUP/groups/n2bin_gu/BIO511-2026/data/fastq ./data
```
:::
...
## Explanation
The bottom line in a FASTQ file represents the quality scores of each base in the corresponding sequence. These scores are encoded using ASCII characters, where each character corresponds to a specific quality value. Higher quality scores indicate more confidence in the accuracy of the base call.

Paired-end reads can be identified by their filenames (e.g., `_R1` and `_R2` suffixes) and by the fact that they appear in the same order in their respective files. Each read in the first file has a corresponding mate in the second file.
:::
:::
...
`fastp` is a modern version of what FastQC used to do (in case youve used that before), it is an all-in-one preprocessing tool that combines quality assessment, adapter trimming, and quality filtering in a single program. Unlike the traditional `FastQC` + `TrimGalore` workflow, fastp provides integrated analysis with excellent performance. As you likely remember, we set up fastp on your computer locally last practical, so lets use it now. 

## Fastp analysis

This step can be done on your local computer, since we installed `fastp` in the software practical. Make sure you download the data with scp first! 
...
# View the HTML report in your browser (e.g., open sample1_fastp_report.html)
```
...
fastp -i sample1_R1.fastq.gz -I sample1_R2.fastq.gz -o sample1_R1_filtered.fastq.gz -O sample1_R2_filtered.fastq.gz -h sample1_fastp_report.html -j sample1_fastp_report.json --cut_right --cut_mean_quality 30 --length_required 50
```
::: {.callout-note}
## Key Questions
1. Did this improve or change the quality of the reads?
2. We set a minimum length on bp, why might this be important?

::: {.callout-tip collapse="true"}
### Explanation
Generally as reads extend, the quality of base calls tends to decrease towards the 3' end. This is why trimming low-quality bases from the ends can improve read quality. Additionally, removing very short reads helps to reduce noise and potential mapping errors in downstream analyses. 
:::
...
#SBATCH -t 01:00:00
#SBATCH --output=path/to/logs/kraken2_%j.out  # Standard output OBS: Change this to your actual log directory
#SBATCH --error=path/to/logs/kraken2_%j.err   # Standard error OBS: Change this to your actual log directory

# Preset path for container and database
CONTAINER_PATH="/cephyr/NOBACKUP/groups/n2bin_gu/BIO511-2026/containers/kraken2.sif"
DB_PATH="/cephyr/NOBACKUP/groups/n2bin_gu/BIO511-2026/refs/kraken2db"

# These ones need to be set by you 
...
# Identify sample (change this to run the rest)
sample="A210"
...
MultiQC works best when all results are organized in a clear directory structure. Let's organize our outputs properly. Remember how we structured our workspaces, but now we want the fastp and kraken2 results to be neatly placed into separate subdirectories within a common directory that we can point `multiqc` to.
...
# Verify all files are present (tree is also fine)
