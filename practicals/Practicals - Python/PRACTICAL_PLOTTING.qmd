...
# Exercises
Let’s start with doing something quite similar to what we went through in today’s presentation. For this exercise you’ll need the following external libraries: pandas, matplotlib, seaborn, plotly and plotnine. You can use the environment file provided for the practice dugga (available on Canvas) or make your own environment if you prefer. Start the code with setting up a suitable environment for this practical.
...
Create two (or more) plots of your own choosing to visualize the penguins data. Use seaborn and try to subplot them together. Create a palette or use one of seaborn’s pre-existing palettes to separate the data by any factor you think is appropriate. (island, sex, species, etc.. )
...
Read the data `CNV_log2_skin_melanoma.csv` from the practice dugga (available on Canvas) using pandas. 
